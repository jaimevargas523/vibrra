...
router.get("/kpis", async (req, res) => {
    try {
        ...
        const totalSesiones = negociosSnap.docs.reduce((sum, d) => sum + (d.data().totalSesiones ?? 0), 0);
        const totalCanciones = negociosSnap.docs.reduce((sum, d) => sum + (d.data().totalCanciones ?? 0), 0);
