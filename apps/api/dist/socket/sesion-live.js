...
/* ------------------------------------------------------------------ */
function anfitrionRoom(uid) {
    return `anfitrion:${uid}`;
}
function negocioRoom(negocioId) {
    return `negocio:${negocioId}`;
}
function sesionRoom(sesionId) {
    return `sesion:${sesionId}`;
}
/**
 * Register event handlers for a single socket in the /sesion namespace.
 */
export function registerSessionHandlers(nsp, socket) {
    const uid = socket.data["uid"] ?? "anonymous";
    ...
    socket.on("add-song", (data) => {
        addSongToQueue(data.sessionId, data.titulo, data.artista, data.solicitadoPor, data.precio);
        // Broadcast updated queue → cola:updated
 ...
 */
export function startMockEmitter(nsp) {
    const mockSongs = [
    ...
    setInterval(() => {
        ...
        addSongToQueue(session.id, pick.titulo, pick.artista, `Simulado-${idx + 1}`, [5000, 7000, 10000][idx % 3]);
