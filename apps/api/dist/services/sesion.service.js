...
export function getAllSessions(page, pageSize) {
...
export function getRecentSessions(limit) {
    const all = currentSession
        ? [currentSession, ...pastSessions]
        : [...pastSessions];
    return all.slice(0, limit);
}
export function startSession(establecimientoId) {
    currentSession = {
        id: `ses-${Date.now()}`,
        hostUid: "mock-host-uid-001",
        establecimientoId,
        establecimientoNombre: establecimientoId === "est-001"
            ? "Bar La Terraza"
