...
export function getPendingQueue(sessionId) {
    return getQueue(sessionId).filter((s) => s.estado === "pendiente" || s.estado === "reproduciendo");
}
export function addSongToQueue(sessionId, titulo, artista, solicitadoPor, precio) {
    const newSong = {
        id: `song-${Date.now()}`,
        sessionId,
        titulo,
...
export function nextSong(sessionId) {
    // Mark current "reproduciendo" as "reproducida"
    const playing = mockQueue.find((s) => s.sessionId === sessionId && s.estado === "reproduciendo");
    if (playing) {
        playing.estado = "reproducida";
        playing.reproducidaEn = new Date().toISOString();
    }
    // Promote next pending
    const next = mockQueue.find((s) => s.sessionId === sessionId && s.estado === "pendiente");
    if (next) {
